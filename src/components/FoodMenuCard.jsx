...
const FoodMenuCard = ({ isDarkMode, setIsDarkMode, toggleTheme }) => {
  ...
  const sendWhatsapp = () => {
    if (summaryText.trim() === "") {
      alert("Summary is empty. Please generate the summary before sending.");
      return;
    }
  
    const phoneNumber = "7296984944"; // Replace with the desired phone number
    const whatsappURL = `https://api.whatsapp.com/send?phone=${phoneNumber}&text=${encodeURIComponent(summaryText)}`;
  ...
  return (
    <>
      <div className={`flex flex-col lg:flex-row h-screen ${isDarkMode ? 'bg-black' : 'bg-gray-50'}`}>
        {/* Left Panel */}
        <div className={`lg:w-2/3 w-full p-6 overflow-y-auto ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
          {/* Header */}
          <div className="flex flex-col md:flex-row justify-between items-center mb-6 space-y-4 md:space-y-0">
            ...
            <input
              ...
              className={`w-full md:w-1/3 ${isDarkMode ? 'text-gray-50' : 'text-black'} px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-400`}
              value={searchTerm}
          ...
          <div className="flex space-x-4 mb-6 overflow-x-auto py-2">
            {["all", "main", "drinks", "desserts"].map((category) => (
              <button
                ...
                className={`px-4 py-2 rounded-full border transition-colors whitespace-nowrap ${currentCategory === category ? 'bg-blue-500 text-white' : 'bg-white text-black'
                  }`}
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProducts.map((product) => (
              <div
                key={product.id}
                className={`${isDarkMode ? 'bg-black shadow-white' : 'bg-white'
        ...
        {/* Right Panel */}
        <div
          className={`lg:w-1/3 w-full ${isDarkMode ? 'bg-black text-white' : 'bg-gray-50'} shadow-lg p-6 flex flex-col`}
        >
          ...
          <div className="flex-1 overflow-y-auto mb-4">
            <div className="space-y-2">
              {cart.map((item) => (
                <div
                  key={item.id}
                  className={`flex items-center justify-between ${isDarkMode ? 'bg-black text-white' : 'bg-gray-50'
                    } shadow-sm p-4 rounded-lg border`}
                >
                  <div>
